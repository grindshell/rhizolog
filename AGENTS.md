...
chaotically, which is exactly what it should track.
...
- **Single-user by default**: a wiki with **no accounts is open**, meaning no
  sign-in, nothing refused, every request treated as the one user. Creating the
  first account under `.rhizolog/users/` is what turns authentication on, which
  is how an instance gets served over a network. Once there are accounts, a
  page's frontmatter says who it is for (`public`, `internal`, `restricted` or
  `private`), and that is applied by one SQL predicate every page-returning
  query pastes in, not by a check in the page handler. The UI is an admin
...
does not have. daisyUI is deliberately absent until the demo needs it, because
the landing page's design is its own and the dashboard's component library has
no business styling it.
...
build somewhere else, in CI or a fresh checkout, needs
...
- **One repo, one `.git`.** Scaffolding tools like to create nested git repos
  ...
  creates a `.git` inside `backend/` or elsewhere, delete it, or the root
...
**No em dashes.** Not in documentation, not in code comments, not in doc
comments, not in UI strings, not in commit messages. They have become the most
recognisable tell of machine-written prose, and the writing in this project is
one of the things it is asking to be taken seriously for. A colon, a semicolon,
a full stop or a rewritten clause says the same thing, and the rewrite is
usually better than the dash was.

Three deliberate exceptions, every one of them a specimen rather than prose:

- The encoding warnings under Environment notes, which use a literal em dash to
  show what ANSI round-tripping and Latin-1 decoding do to it. Take the dash out
  and the warning is about nothing.
- `example-wiki/`, which is fixture content rather than this project's voice,
  and which this file already says not to edit.
- Anything generated. `frontend/openapi.json` and `frontend/src/api/schema.d.ts`
  are outputs; fix the Rust doc comment upstream and regenerate.

`Select-String -Pattern ([char]0x2014) -Path <files>` finds any that have crept
back in, and so does `rg` given the character directly.
...
because `desktop/` depends on `rhizolog` with it enabled and there is only one
build of the library. Which means those two also need `pnpm build` behind them.
...
- **Never edit a source file by round-tripping it through the shell.**
  ...
  (`—` becomes `â€”`), and `-Encoding utf8` writes a BOM. A BOM has already
  caused one real bug here: it hid a page's frontmatter, since the text no
  longer started with `---`.
...
- **Never save a downloaded file through a PowerShell string.** `curl` in 5.1
  ...
  the obvious way to refresh the spec, mangles every non-ASCII byte sequence
  in it: an em dash goes from `E2 80 94` to `C3 A2 C2 80 C2 94`. The result is
  still valid JSON and still one line, so the diff looks like a normal
  regeneration and nothing catches it. Use `cargo run --example dump-openapi`,
  which sidesteps HTTP entirely; failing that, download bytes and write them
  verbatim (`WebClient.DownloadData` + `[System.IO.File]::WriteAllBytes`), then
  check that the first non-ASCII bytes are the ones you expect. The same
  applies to `>` and `Out-File` generally, which re-encode and add a BOM.
  `git show HEAD:f > tmp` does not give you the committed bytes;
  `git checkout HEAD -- f` does.
- **Never pass a quoted string straight to a native command.** 5.1 re-parses an
  ...
  `git commit -F <file>`, which carries quotes and non-ASCII characters through
  intact.
